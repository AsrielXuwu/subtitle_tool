...
def build_style_tab(parent, font_v, size_v, col_v, ocol_v, mv_v, mlr_v, out_v, align_v, shad_v, bold_v, ita_v, alpha_v=None, oalpha_v=None):
    # 【核心修改点】使用总容器隔离外部 grid 布局，内部全部采用紧凑的流式 pack 布局
    container = ttk.Frame(parent)
    # 利用 rowspan=3 完美覆盖原代码的 3 行占位，保证不影响外部挂载的分辨率等其他组件
    container.grid(row=0, column=0, rowspan=3, columnspan=8, sticky="w")
    
    # ---- 第一行：字体与排版 ----
    r1 = ttk.Frame(container)
    r1.pack(fill=tk.X, pady=(5, 2), anchor="w")
    
    ttk.Label(r1, text="字体:").pack(side=tk.LEFT)
    cb = ttk.Combobox(r1, textvariable=font_v, width=22)
    cb.pack(side=tk.LEFT, padx=(2, 10))
    
    ttk.Label(r1, text="字号:").pack(side=tk.LEFT)
    ttk.Entry(r1, textvariable=size_v, width=4).pack(side=tk.LEFT, padx=(2, 10))
    
    ttk.Checkbutton(r1, text="加粗", variable=bold_v).pack(side=tk.LEFT, padx=2)
    ttk.Checkbutton(r1, text="斜体", variable=ita_v).pack(side=tk.LEFT, padx=(2, 10))
    
    ttk.Label(r1, text="对齐方式:").pack(side=tk.LEFT)
    ttk.Combobox(r1, textvariable=align_v, values=[str(i) for i in range(1,10)], width=3, state="readonly").pack(side=tk.LEFT, padx=(2, 0))

    # ---- 第二行：颜色与特效 ----
    r2 = ttk.Frame(container)
    r2.pack(fill=tk.X, pady=2, anchor="w")
    
    ttk.Label(r2, text="主色(HEX):").pack(side=tk.LEFT)
    c_btn = tk.Button(r2, width=3, bg="#FFFFFF", relief="ridge", command=lambda: choose_color(col_v, c_btn))
    c_btn.pack(side=tk.LEFT, padx=(2, 5))
    if alpha_v is not None:
        ttk.Label(r2, text="透明度:").pack(side=tk.LEFT)
        ttk.Entry(r2, textvariable=alpha_v, width=3).pack(side=tk.LEFT, padx=(2, 10))
        
    ttk.Label(r2, text="描边(HEX):").pack(side=tk.LEFT)
    oc_btn = tk.Button(r2, width=3, bg="#000000", relief="ridge", command=lambda: choose_color(ocol_v, oc_btn))
    oc_btn.pack(side=tk.LEFT, padx=(2, 5))
    if oalpha_v is not None:
        ttk.Label(r2, text="透明度:").pack(side=tk.LEFT)
        ttk.Entry(r2, textvariable=oalpha_v, width=3).pack(side=tk.LEFT, padx=(2, 10))
        
    ttk.Label(r2, text="描边大小:").pack(side=tk.LEFT)
    ttk.Entry(r2, textvariable=out_v, width=3).pack(side=tk.LEFT, padx=(2, 10))
    
    ttk.Label(r2, text="阴影深度:").pack(side=tk.LEFT)
    ttk.Entry(r2, textvariable=shad_v, width=3).pack(side=tk.LEFT, padx=(2, 0))

    # ---- 第三行：边距与调参按钮 ----
    r3 = ttk.Frame(container)
    r3.pack(fill=tk.X, pady=(2, 5), anchor="w")
    
    ttk.Label(r3, text="上下边距:").pack(side=tk.LEFT)
    ttk.Entry(r3, textvariable=mv_v, width=4).pack(side=tk.LEFT, padx=(2, 10))
    
    ttk.Label(r3, text="左右边距:").pack(side=tk.LEFT)
    ttk.Entry(r3, textvariable=mlr_v, width=4).pack(side=tk.LEFT, padx=(2, 10))
    ...
    ttk.Button(r3, text="📐 图形界面调边距", command=lambda: open_visual_adjuster(parent, mv_v, mlr_v, align_v, font_v, size_v, col_v, ocol_v, out_v, bold_v, ita_v, alpha_v, oalpha_v)).pack(side=tk.LEFT, padx=10)
...
ttk.Button(f7_top, text="1. 刷新文件夹", command=load_m7_files).pack(side=tk.LEFT, padx=5)
m7_file_cb = ttk.Combobox(f7_top, textvariable=m7_file_var, state="readonly")
m7_file_cb.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
ttk.Button(f7_top, text="2. 加载文件内容", command=load_m7_content).pack(side=tk.LEFT, padx=5)
ttk.Label(f7_top, text="(支持按住Shift/Ctrl多选行)", foreground="gray").pack(side=tk.LEFT, padx=5)
